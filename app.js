...
function renderCategory(){
  ...
  return shell(`<button class="ghost back" data-action="back-library">← Phân loại</button><div class="section-title"><div><h2>${esc(c.name)}</h2><div class="subtle">Cấp 2 · Chạm/click vào dòng để mở · dùng ↑ ↓ để sắp xếp</div></div><button class="primary" data-action="add-item">+ Dán link</button></div><div class="entry-list">${items.map((item,i)=>`<div class="card entry nbl-click-row" data-open-item="${item.id}" role="button" tabindex="0"><img class="thumb" src="${esc(item.thumbnail||'./icon.svg')}" alt=""><div><h3>${esc(item.title)}</h3><div class="meta">${item.kind==='channel'?'Kênh YouTube':item.kind==='playlist'?'Playlist':'Video'}${item.kind==='playlist'&&item.totalVideos!=null?' · '+item.totalVideos+' video':''}</div></div><div class="row-actions"><button class="iconbtn" title="Đưa lên" data-move-library-item="up" data-item-id="${item.id}" ${i===0?'disabled':''}>↑</button><button class="iconbtn" title="Đưa xuống" data-move-library-item="down" data-item-id="${item.id}" ${i===items.length-1?'disabled':''}>↓</button><button class="iconbtn" data-edit-item="${item.id}">Sửa</button></div></div>`).join('')}</div>${items.length?'':'<div class="empty">Dán link kênh hoặc playlist YouTube để bắt đầu.</div>'}`)
}
function renderItem(){
...
function bind(){
  ...
  document.querySelectorAll('[data-open-category]').forEach(el=>{
    el.onclick=e=>{if(e.target.closest('button,a,input,textarea,select,label'))return;view.categoryId=el.dataset.openCategory;render()};
    el.onkeydown=e=>{if((e.key==='Enter'||e.key===' ')&&!e.target.closest('button,a,input,textarea,select,label')){e.preventDefault();view.categoryId=el.dataset.openCategory;render()}};
  });
  ...
  document.querySelectorAll('[data-open-item]').forEach(el=>{
    el.onclick=e=>{if(e.target.closest('button,a,input,textarea,select,label'))return;view.itemId=el.dataset.openItem;render()};
    el.onkeydown=e=>{if((e.key==='Enter'||e.key===' ')&&!e.target.closest('button,a,input,textarea,select,label')){e.preventDefault();view.itemId=el.dataset.openItem;render()}};
  });
