...
(function(){
  const NBL_QUEUE_LIMIT = 50; // YouTube temporary watch_videos queue is practically limited to 50 items.

  // ---- Migrate existing state without touching the old storage key ----
  ...
  // ---- Vivaldi-first external opening ----
  // Vivaldi's documented/reliable iOS path is to make it the default browser.
  // We therefore open a normal HTTPS URL in a new external browsing context.
  // This avoids relying on an undocumented deep-link format that can break query strings.
  ...
  // ---- Sync custom listening playlists as well ----
  ...
  function tempQueueUrl(videos,start=0){
    ...
    return `https://www.youtube.com/watch_videos?video_ids=${ids.map(encodeURIComponent).join(',')}`;
  }
  ...
  function showCreateQueue(){
    const root=modal(`<h2>Tạo danh sách phát</h2>
      <div class="field"><label>Tên danh sách</label><input id="qp-name" class="input" placeholder="Ví dụ: Nghe khi đi làm"></div>
      <div class="field"><label>Dán các link video YouTube (mỗi link một dòng)</label><textarea id="qp-links" class="input nbl-textarea" placeholder="https://youtube.com/watch?v=...\nhttps://youtu.be/...\n..."></textarea><small class="subtle">Có thể tạo danh sách trước rồi thêm video sau. Thứ tự dán chính là thứ tự phát.</small></div>
      <div class="modal-actions"><button class="ghost" data-close>Hủy</button><button class="primary" data-save>Tạo danh sách</button></div>`);
  ...
  function showAddVideos(q){
    const root=modal(`<h2>Thêm video vào ${esc(q.name)}</h2>
      <div class="field"><label>Dán nhiều link YouTube</label><textarea id="qp-links" class="input nbl-textarea" autofocus placeholder="Mỗi link một dòng"></textarea><small class="subtle">Link trùng trong danh sách sẽ tự bỏ qua.</small></div>
      <div class="modal-actions"><button class="ghost" data-close>Hủy</button><button class="primary" data-save>Thêm video</button></div>`);
    root.querySelector('[data-close]').onclick=()=>root.remove();
    root.querySelector('[data-save]').onclick=async function(){
      const ids=parseManyVideoIds(root.querySelector('#qp-links').value);
  ...
  function renderQueueList(){
    ...
    return shell(`<div class="section-title"><div><h2>Danh sách phát</h2><div class="subtle">Tự gom nhiều link video từ các kênh khác nhau</div></div><button class="primary" data-q-action="create">+ Tạo danh sách</button></div>
      <div class="notice">Khi bấm phát, News By Listening tạo một hàng đợi YouTube tạm thời theo đúng thứ tự anh sắp xếp rồi mở bằng Vivaldi. Mỗi lượt phát liên tục tối đa ${NBL_QUEUE_LIMIT} video; danh sách lưu trong app có thể dài hơn.</div>
      <div class="grid nbl-queue-grid" style="margin-top:12px">${qs.map(q=>`<div class="card category-card" data-q-open="${q.id}"><div class="card-actions"><button class="iconbtn" data-q-rename="${q.id}">Sửa</button></div><div class="category-index">DANH SÁCH PHÁT</div><h3>${esc(q.name)}</h3><div class="count">${(q.videos||[]).length} video</div><div class="toolbar nbl-card-toolbar"><button class="primary" data-q-play="${q.id}">▶ Phát</button></div></div>`).join('')}</div>
      ${qs.length?'':'<div class="empty" style="margin-top:12px">Chưa có danh sách phát. Tạo một danh sách rồi dán các link video cần nghe.</div>'}`);
  }

  function renderQueueDetail(q){
    const vs=q.videos||[];
    return shell(`<button class="ghost back" data-q-action="back">← Danh sách phát</button>
      <div class="section-title"><div><h2>${esc(q.name)}</h2><div class="subtle">${vs.length} video · kéo thả trên PC hoặc dùng ↑ ↓ để đổi thứ tự</div></div><button class="primary" data-q-action="add-videos">+ Dán video</button></div>
      <div class="toolbar"><button class="primary" data-q-action="play-all">▶ Phát từ đầu</button><button class="ghost" data-q-action="rename">Đổi tên</button><button class="danger" data-q-action="delete-queue">Xóa danh sách</button></div>
      ${vs.length>NBL_QUEUE_LIMIT?`<div class="notice warn">Danh sách có ${vs.length} video. Một lượt YouTube tạm thời phát tối đa ${NBL_QUEUE_LIMIT} video. Anh có thể bấm “Nghe từ đây” ở video #${NBL_QUEUE_LIMIT+1} để phát phần tiếp theo.</div>`:''}
      <div class="video-list nbl-queue-videos">${vs.map((v,i)=>`<div class="card video nbl-q-video" draggable="true" data-q-index="${i}"><img class="thumb" src="${esc(v.thumbnail||ytThumb(v.videoId))}" alt=""><div><div class="video-index">#${i+1}</div><h3>${esc(v.title)}</h3><div class="meta">${esc(v.channelTitle||'')}${v.duration?' · '+fmtDuration(v.duration):''}</div></div><div class="row-actions"><button class="iconbtn" data-q-move="up" data-index="${i}" ${i===0?'disabled':''}>↑</button><button class="iconbtn" data-q-move="down" data-index="${i}" ${i===vs.length-1?'disabled':''}>↓</button><button class="primary" data-q-play-index="${i}">▶ Nghe từ đây</button><button class="danger" data-q-remove="${i}">Xóa</button></div></div>`).join('')}</div>
      ${vs.length?'':'<div class="empty">Dán các link video YouTube để bắt đầu.</div>'}`);
  }

  function renderQueues(){
    const q=currentQueue();
    return q?renderQueueDetail(q):renderQueueList();
  }

  // ---- Settings: Vivaldi + backend status ----
  renderSettings = function(){
    ...
    return shell(`<div class="section-title"><div><h2>Cài đặt</h2><div class="subtle">Vivaldi là trình duyệt phát mặc định</div></div></div>
      <div class="card form">
        <div class="field"><label>Trình duyệt phát</label><div><span class="badge green">Vivaldi</span></div><small class="subtle">News By Listening mở URL HTTPS ra trình duyệt mặc định của iPhone để tránh lỗi deep-link. Hãy đặt Vivaldi làm Default Browser.</small></div>
        <div class="notice"><b>Cấu hình Vivaldi một lần:</b><br>1) iPhone Settings → Apps → Default Apps → Browser App → Vivaldi.<br>2) Vivaldi → Settings → General → Allow media playback in background = ON.<br>3) Vivaldi → Settings → Privacy and security → Don't open links in external apps = ON.<br>4) Ad and Tracker Blocker → Block Trackers and Ads.</div>
        <button class="ghost" data-q-action="test-vivaldi">Mở thử YouTube bằng Vivaldi</button>
        <div class="hr"></div>
        <div class="field"><label>YouTube Data API</label><div><span class="badge ${statusClass}">${esc(statusText)}</span></div><small class="subtle">API key nằm ở backend Supabase; PC/iPhone không giữ key.</small></div>
        ...
        <div class="notice">Tốc độ được app ghi nhớ để nhắc đúng thói quen; video thực tế phát trong Vivaldi/YouTube nên app không ép được playback speed hoặc EQ của trình phát ngoài.</div>
        <div class="hr"></div><div class="subtle">Phiên bản 1.3.0 · Vivaldi + Danh sách phát tùy chỉnh</div>
      </div>`);
  };
  ...
  // ---- Click handling for new UI ----
  document.addEventListener('click',function(e){
    const navBtn=e.target.closest('[data-nav="queues"]');
    if(navBtn){
      e.preventDefault(); e.stopImmediatePropagation();
      view.tab='queues'; view.categoryId=null; view.itemId=null; view.queueId=null; render(); return;
    }
    ...
    if(remove){e.preventDefault();e.stopImmediatePropagation();const q=currentQueue();const i=Number(remove.dataset.qRemove);if(q&&q.videos?.[i]&&confirm(`Xóa “${q.videos[i].title}” khỏi danh sách?`)){q.videos.splice(i,1);q.updatedAt=new Date().toISOString();save();render();}return;}

    const move=e.target.closest('[data-q-move]');
    if(move){e.preventDefault();e.stopImmediatePropagation();const q=currentQueue();const i=Number(move.dataset.index);const j=move.dataset.qMove==='up'?i-1:i+1;if(q&&j>=0&&j<q.videos.length){const [v]=q.videos.splice(i,1);q.videos.splice(j,0,v);q.updatedAt=new Date().toISOString();save();render();}return;}
  },true);

  // ---- Desktop drag/drop ordering ----
  let dragIndex=null;
  ...
  // Update sync wording so it is clear custom playlists are included.
  ...
  // Re-render once so the new nav/header appears immediately after update.
  render();
})();
